...
let menu=null;function showMenu(ev,id){if(menu)menu.remove();menu=document.createElement("div");menu.className="context-menu";menu.innerHTML=`<button data-act="rename">Переименовать</button><button data-act="delete" class="danger">Удалить</button>`;document.body.appendChild(menu);menu.style.left=ev.pageX+"px";menu.style.top=ev.pageY+"px";menu.style.display="block";menu.onclick=(e)=>{if(e.target.dataset.act==="rename")renameChat(id);if(e.target.dataset.act==="delete")deleteChat(id);menu.remove();menu=null};window.addEventListener("click",()=>{if(menu)menu.remove();menu=null},{once:true})}
